...
GuitarChordSelectorDialog::parseChordFile(const QString& chordFileName)
{
    ...
    if (!ok)
        QMessageBox::critical(nullptr, tr("梨音教学工具"), tr("couldn't open file '%1'").arg(handler.errorString()));
...
// RG_DEBUG << "  parsed OK, without crashing!  W00t!";
    ...
    if (!ok)
        QMessageBox::critical(nullptr, tr("梨音教学工具"), tr("couldn't parse chord dictionary : %1").arg(handler.errorString()));
