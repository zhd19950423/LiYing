...
{
    if (QMessageBox::warning(this, tr("梨音教学工具"), tr("This will remove all triggered segments from the whole composition.  Are you sure?"), QMessageBox::Yes|QMessageBox::Cancel, QMessageBox::Cancel ) != QMessageBox::Yes )
        return ;
...
TriggerSegmentManager::slotDelete()
{
    ...
    if (item->getUsage() > 0) {
        if (QMessageBox::warning(this, tr("梨音教学工具"), tr("This triggered segment is used %n time(s) in the current composition.  Are you sure you want to remove it?", "", item->getUsage()),
...
TriggerSegmentManager::slotPasteAsNew()
{
    ...
    if (clipboard->isEmpty()) {
        QMessageBox::information(this, tr("梨音教学工具"), tr("Clipboard is empty"));
        return ;
    }
